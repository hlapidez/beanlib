 ...
 */
public abstract class ExtensibleTreeMap<K, V> extends AbstractMap<K, V> 
    implements SortedMap<K, V>, Cloneable, java.io.Serializable 
{
    ...
    private class SubMap extends AbstractMap<K, V> implements SortedMap<K, V>,
        ...
        public K firstKey() {
            ExtensibleTreeMap.NodeEntry<K, V> e = fromStart ? firstEntry()
                    : getCeilEntry(fromKey);
            K first = key(e);
        ...
        public K lastKey() {
            ExtensibleTreeMap.NodeEntry<K, V> e = toEnd ? lastEntry()
                    : getPrecedingEntry(toKey);
            K last = key(e);
        ...
        private class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
            ...
            @Override
            public Iterator<Map.Entry<K, V>> iterator() {
                return new SubMapEntryIterator((fromStart ? firstEntry()
                        : getCeilEntry(fromKey)), (toEnd ? null
                        : getCeilEntry(toKey)));
            }
        }
    ...
    private class SubMapEntryIterator extends
     ...
     */
    private final NodeEntry<K, V> buildFromSorted(int level, int lo, int hi,
        ...
        if (mid < hi) {
            NodeEntry<K, V> right = buildFromSorted(level + 1, mid + 1, hi,
